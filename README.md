...
> ### ⚠ 지금 남은 단 하나의 작업: 메일 확인 링크 클릭 (30초)
> 메일 전송 경로(FormSubmit)를 켜 두었고, **yerim.oh@vision.snu.ac.kr 로 "Activate Form" 확인 메일이 발송된 상태**입니다.
> 그 링크를 한 번 누르면 그때부터 설문 응답이 이 주소로 자동 전달됩니다. 서버도 구글 계정도 필요 없습니다.
> **활성화는 설문을 뿌리기 전에 끝내세요.** 활성화 전에 들어온 응답은 유실되지는 않지만(응답자 브라우저 보류함에 보관), 그 사람이 링크를 다시 열어야 전송됩니다.
> 활성화 후 FormSubmit이 주는 해시 주소(`formsubmit.co/ajax/xxxxx`)로 바꾸면 메일 주소를 코드에 노출하지 않아도 됩니다.
