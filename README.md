...
**AI가 수행한 연구는 어떤 흔적을 남기는가** — 몰드 커뮤니티 서베이 (GitHub Pages 정적 앱).
...
| 프라이밍 통제 | 예시를 첫 화면에 노출 | 무프라이밍 자유회상(S2b)을 카드 앞에 배치 + **이후 되돌아가기 차단** (예시는 합의된 2장만: 논증그래프·그림색) |
...
구글폼으로 불가능해서 커스텀 페이지로 만든 것: 되돌아가기 차단, 카드 순서 무작위화+순서 로깅, 페이지별 체류시간 로깅, 조건 분기, `?src=` 기관 태깅, ko/en 전환.
...
- [ ] 응답 후 `responses` 시트의 `totalMinutes` < 4 는 분석 제외 규칙 그대로인지
...
- **품질 필터**: honeypot 채워짐 / 총 체류 4분 미만 / r_p1 20자 미만 우회 시도 → 제외.
